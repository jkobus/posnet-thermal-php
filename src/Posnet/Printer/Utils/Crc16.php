...
namespace Posnet\Printer\Utils;

/**
 * Calculate CRC16-? checksum
 * @todo what kind of crc16 is it ?
 */
class Crc16
{
    public static $crc16htab = array(
    ...
    public static function calculate($data)
    {
        ...
        foreach (str_split($data, 1) as $char) {
            $byte = ord($char);
            $index = $hi ^ $byte;
            $hi = $lo ^ self::$crc16htab[$index];
            $lo = self::$crc16ltab[$index];
        }

        return dechex(($hi << 8) | $lo);
    }
